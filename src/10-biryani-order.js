...
export function biryaniBatchProcessor(orders) {
  ...
  for (const order of orders) {
    // Skip invalid orders - non-number, decimal, 0, negative
    ...
    // do...while - har order mein at least 1 batch toh banega hi
    do {
      const batch = Math.min(remaining, 5); // max 5 plates per batch
      totalBatches++;
      totalPlates += batch;
      remaining -= batch;
    } while (remaining > 0);
